...
class Evaluator:
    ...
    def evaluate_one(self, db_name, gold, predicted):
        ...
        try:
            ...
            predicted = predicted.replace('LIKE"', 'like "')
            predicted = predicted.replace('"AND', '" AND')
            predicted = predicted.replace('"OR', '" OR')
            predicted = predicted.replace('"INTERSECT', '" INTERSECT')
            predicted = predicted.replace('"EXCEPT', '" EXCEPT')
            predicted = predicted.replace('"GROUP', '" GROUP')
            predicted = predicted.replace('"HAVING', '" HAVING')
        except:
            print('')
        ...
        if self.etype in ["all", "match"]:
            ...
            if exact_score == 0:
                with open('errors.txt', 'at') as output:
