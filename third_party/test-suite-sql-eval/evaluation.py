...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint):
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            p_str = p[0]
            p_str = p_str.replace('="', '= "')
            p_str = p_str.replace('LIKE"', 'like "')
            p_str = p_str.replace('"AND', '" AND')
            p_str = p_str.replace('"OR', '" OR')
            p_str = p_str.replace('"INTERSECT', '" INTERSECT')
            p_str = p_str.replace('"EXCEPT', '" EXCEPT')
            p_str = p_str.replace('"GROUP', '" GROUP')
            p_str = p_str.replace('"HAVING', '" HAVING')
